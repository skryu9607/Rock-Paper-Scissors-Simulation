...
def step1_baseline_validation(initial_pop, K=200, max_steps=5000):
    '''
    ...
    from rps_pctl import (
        S, S_index, nS, transient, JOINT_ACTIONS, T as T_mat,
...
def step2_estimate_alpha_effective(initial_pop, K=300, max_steps=5000):
    '''
...
def step3_model_check_with_alpha(all_results, initial_pop, T_horizon=300):
    '''
    ...
    from rps_pctl import (
        T as T_precomputed, JOINT_ACTIONS as JA,
        pctl_bounded_coexist, expected_extinction_time
    )
    ...
    E_pmax = expected_extinction_time(mode="max")
    E_pmin = expected_extinction_time(mode="min")
    E_prand = expected_extinction_time(mode="random")
...
if __name__ == "__main__":
    ...
    sim_et, model_et = step1_baseline_validation(
        initial_pop=INITIAL_POP, K=30, max_steps=1000
    )
