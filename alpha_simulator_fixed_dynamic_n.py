'''
Bridge: Agent-Level Simulator <-> Population-Level Model Checking
All simulators are headless (no pygame display).
'''
...
def load_rps_pctl(total_pop):
    os.environ["RPS_TOTAL_POP"] = str(total_pop)

    if "rps_pctl" in sys.modules:
        module = sys.modules["rps_pctl"]
        if getattr(module, "N", None) != total_pop:
            module = importlib.reload(module)
    else:
        module = importlib.import_module("rps_pctl")

    if getattr(module, "N", None) != total_pop:
        raise RuntimeError("rps_pctl.py was loaded with the wrong population size")
...
def step1_baseline_validation(initial_pop=(4, 4, 4), K=200, max_steps=5000):
    ...
    rps_model = load_rps_pctl(N)
    E_rand = rps_model.expected_extinction_time(mode="random")
    s0 = initial_pop
    s0_idx = rps_model.S_index[s0]
...
def step2_estimate_alpha_effective(initial_pop=(40, 40, 40), K=300, max_steps=5000):
    ...
    for policy in policies:
        ...
        RS_count = defaultdict(int)            # R→S conversions while in state s
        SP_count = defaultdict(int)            # S→P conversions while in state s
        PR_count = defaultdict(int)            # P→R conversions while in state s
...
def step3_model_check_with_alpha(all_results, initial_pop=(4, 4, 4), T_horizon=300):
    ...
    # Also compute P_max and P_min for reference
    rps_model = load_rps_pctl(N_pop)

    V_pmax, _ = rps_model.pctl_bounded_coexist(T_horizon, mode="max")
    V_pmin, _ = rps_model.pctl_bounded_coexist(T_horizon, mode="min")
    V_prand, _ = rps_model.pctl_bounded_coexist(T_horizon, mode="random")
    E_pmax = rps_model.expected_extinction_time(mode="max")
    E_pmin = rps_model.expected_extinction_time(mode="min")
    E_prand = rps_model.expected_extinction_time(mode="random")

    print(f"\n  Population model bounds (from rps_pctl.py):")
    ...
    for policy in all_results:
        ...
        if e_max > e_min:
            ...
            print(f"  {policy:<12} {e:<15.2f} {position*100:.1f}% along P_min→P_max  "
...
if __name__ == "__main__":
    ...
    sim_et, model_et = step1_baseline_validation(
        initial_pop=INITIAL_POP, K=30, max_steps=500
    )
