...
<out>/previews/preview_episode_XXXXX.gif
    Optional visual sanity-check animations for selected deterministic episodes.
    Use --preview-count/--preview-episode-ids and --preview-only to inspect a few
    runs before launching a large Monte Carlo batch.
...
type_images = {
    0: plt.imread(asset_dir / "Rock.png"),
    1: plt.imread(asset_dir / "Scissors.png"),
    2: plt.imread(asset_dir / "Paper.png"),
}
...
def simulate_episode(
    ...
    for step in range(1, max_steps + 1):
        ...
        if alive < 3:
            ...
            if pop[0] == 0:      # R extinct -> S vs P -> S wins
                winner = "S"
            elif pop[1] == 0:    # S extinct -> R vs P -> P wins
                winner = "P"
            elif pop[2] == 0:    # P extinct -> R vs S -> R wins
                winner = "R"
...
def render_preview_gif(
    ...
    """Render a compact visual sanity check of one deterministic episode.

    The preview uses the *same trajectory arrays* produced by simulate_episode(),
    so it is not a separate visual simulator with different dynamics.
    """
    import matplotlib
    matplotlib.use("Agg", force=True)
    import matplotlib.pyplot as plt
    ...
    types = trajectory["type"]
    populations = trajectory["population"]

    n_saved = len(steps)
    if n_saved == 0:
        return
    frame_stride = max(1, math.ceil(n_saved / max(1, max_frames)))
    frame_ids = list(range(0, n_saved, frame_stride))
    if frame_ids[-1] != n_saved - 1:
        frame_ids.append(n_saved - 1)

    # R, S, P colors are deliberately stable across every preview.
    type_colors = np.asarray(["#d62728", "#2ca02c", "#1f77b4"], dtype=object)
    type_labels = {0: "Rock", 1: "Scissors", 2: "Paper"}
    ...
    def draw(frame_number: int):
        ...
        for type_id in (0, 1, 2):
            ...
            pts = p[mask]
            for x, y in pts:
                imagebox = OffsetImage(type_images[type_id], zoom=0.35)
                ab = AnnotationBbox(
                    imagebox,
                    (x, y),
                    frameon=False,
            ...
            if show_velocity:
                vv = v[mask]
                ax.quiver(
                    pts[:, 0], pts[:, 1], vv[:, 0], vv[:, 1],
                    angles="xy", scale_units="xy", scale=0.18,
                    width=0.0022, alpha=0.45,
                    color=type_colors[type_id],
                )

        ax.set_xlim(0, WIDTH)
        # Simulator screen coordinates increase downward; invert y to match it.
        ...
        ax.grid(alpha=0.15)
        ax.set_title(
            f"Episode {episode_id} | step={int(steps[idx])} | "
            f"R/S/P={tuple(int(x) for x in pop)}\n"
            f"first extinction={summary.first_extinction_step}, "
            f"winner={summary.winner}, terminal={summary.terminal_step}"
        )

    animation = FuncAnimation(fig, draw, frames=len(frame_ids), interval=1000 / max(1, fps))
    output_path.parent.mkdir(parents=True, exist_ok=True)
    animation.save(output_path, writer=PillowWriter(fps=max(1, fps)))
    plt.close(fig)


def generate_previews(args, output_dir: Path):
    """Generate deterministic GIFs for selected episode IDs.

    These use exactly the same episode ID -> seed -> initial composition mapping
    as the HDF5 batch, so a preview_episode_00042.gif corresponds to episode_00042
    in the baseline dataset generated with the same command-line arguments.
    """
    if args.preview_episode_ids:
        preview_ids = sorted(set(args.preview_episode_ids))
    else:
        preview_ids = list(range(min(args.preview_count, args.episodes)))

    if not preview_ids:
        return []

    for ep_id in preview_ids:
        if ep_id < 0 or ep_id >= args.episodes:
            raise SystemExit(
                f"preview episode id {ep_id} is outside [0, {args.episodes - 1}]"
            )

    preview_dir = output_dir / "previews"
    preview_dir.mkdir(parents=True, exist_ok=True)
    paths = []
    print("Generating visual preview episodes...")
    for ep_id in preview_ids:
        seed = args.seed_base + ep_id
        ...
        summary, trajectory = simulate_episode(
            ep_id,
            seed,
            initial_pop,
            args.policy,
        ...
        path = preview_dir / f"preview_episode_{ep_id:05d}.gif"
        render_preview_gif(
            ep_id,
            summary,
            trajectory,
            ...
            show_velocity=not args.preview_hide_velocity,
        )
        paths.append(path)
        print(
            f"  preview {ep_id:05d}: init={initial_pop}, "
            f"winner={summary.winner}, coexist={summary.first_extinction_step} -> {path}"
        )
    return paths
...
def parse_args():
    ...
    p.add_argument(
        ...
        type=int,
        default=0,
        help="Render the first N episode IDs as GIF sanity checks (default: 0).",
    )
    p.add_argument(
        "--preview-episode-ids",
        nargs="*",
        type=int,
        default=None,
        help="Specific episode IDs to render; overrides --preview-count.",
    )
    p.add_argument(
        "--preview-only",
        action="store_true",
        help="Generate preview GIFs and exit without launching the bulk HDF5 batch.",
    )
    p.add_argument(
        "--preview-fps",
        type=int,
        default=20,
        help="GIF playback rate (default: 20).",
    )
    p.add_argument(
        "--preview-max-frames",
        type=int,
        default=250,
        help="Maximum rendered frames per preview; long trajectories are subsampled.",
    )
    p.add_argument(
        "--preview-hide-velocity",
        action="store_true",
        help="Hide velocity arrows in preview GIFs.",
    )
...
def main():
    ...
    if args.preview_max_frames <= 1:
        raise SystemExit("--preview-max-frames must be > 1")

    output_dir = Path(args.output)
    output_dir.mkdir(parents=True, exist_ok=True)

    preview_paths = generate_previews(args, output_dir)
    if args.preview_only:
        if not preview_paths:
            raise SystemExit("--preview-only requested but no preview episodes were selected")
        print("Preview-only mode complete. Inspect the GIFs before launching the full batch.")
        return
    ...
    if preview_paths:
        print(f"preview GIFs    : {len(preview_paths)} in {(output_dir / 'previews').resolve()}")
    print("=" * 72)
